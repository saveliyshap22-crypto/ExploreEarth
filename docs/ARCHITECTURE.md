...
- Топографический слой использует `tile.openmaps.fr` только для обычного интерактивного просмотра с атрибуцией; предзагрузка и массовое скачивание отсутствуют.
...
Оригинал не изменяется и не отправляется в сеть. Сетевой сервис получает только распознанные текстовые строки. Tesseract создаётся на время одного анализа, после чего очищает постоянный кэш и освобождает память.
